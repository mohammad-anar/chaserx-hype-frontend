...
import { ShoppingCart, User as UserIcon, ChevronDown, QrCode } from "lucide-react";
...
export default function Navbar({ theme = "light", transparent = false }: NavbarProps) {
    ...
    const liveProfile = profileResponse?.data || profileResponse;
    ...
    // Priority: live API profile > Redux slice > localStorage fallback
    const displayName = liveProfile?.name || user?.name || user?.fullName || userName || "User";
    const displayRole = liveProfile?.role || user?.role || userRole || "USER";
    const rawPhoto = liveProfile?.photo || liveProfile?.image || user?.profileImg || user?.avatar || userPhoto || "";
            ...
            >
                <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between transition-all duration-300 ${
                    ...
                    <div className="flex items-center gap-3 md:gap-4">
                        {/* Member QR Code Button for logged in user */}
                        {isAuthenticated && (
                            <button
                                onClick={() => setIsQrModalOpen(true)}
                                className={`p-2 rounded-full transition-all duration-200 flex items-center justify-center cursor-pointer group ${
                                    isDark ? "hover:bg-white/10 text-white" : "hover:bg-[#FAF0ED] text-[#2C1A14] hover:text-[#C07C4A]"
                                }`}
                                title="My Member Loyalty QR"
                                aria-label="Open Member QR"
                            >
                                <QrCode className="w-[20px] h-[20px] transition-transform duration-200 group-hover:scale-110" />
                            </button>
                        )}
                                        ...
                                        <div className="absolute right-0 mt-3 w-60 rounded-2xl border border-white/10 bg-[#1A0C08] shadow-2xl p-2 z-50 origin-top-right animate-scale-in">
                                            ...
                                            <div className="py-1.5 space-y-1">
                                                ...
                                                <button 
                                                    ...
                                                    className="flex items-center w-full px-4 py-2.5 text-sm rounded-xl hover:bg-white/5 text-[#C07C4A] hover:text-white transition-all font-semibold cursor-pointer text-left gap-2"
                                                >
                                                    <QrCode className="w-4 h-4" />
                                                    <span>My Member QR Code</span>
                                                </button>
                                                <Link 
                                                    href="/rewards" 
                                                    onClick={() => setIsProfileOpen(false)}
